...
(()=>{
  ...
  function currentNarrative(){
    let thesis=null;try{thesis=window.PropertyThesisInvestmentThesis?.build?.()||null;}catch(_e){}
  ...
  function apply(){
    ...
    renameSections();refineReportLabels();
    if(!p)return false;
    const existing=(p.textContent||'').replace(/\s+/g,' ').trim();
    // Preserve the complete thesis produced by the report builder. Older versions
    // replaced it on delayed passes with a shorter synthesized paragraph, causing
    // visible text to disappear after initial render.
    if(existing){box.dataset.currentExecutiveConclusion='7-preserved';return true;}
    const text=currentNarrative();if(!text)return false;
    p.innerHTML=esc(text);box.dataset.currentExecutiveConclusion='7-fallback';return true;
  }

  function wrap(obj,key,flag){const fn=obj?.[key];if(typeof fn!=='function'||fn[flag])return;const wrapped=function(...args){const out=fn.apply(this,args);setTimeout(apply,0);return out;};wrapped[flag]=true;obj[key]=wrapped;}
  function install(){wrap(window.ReportBuilderV1,'renderReport','__currentExecutiveConclusionV7');wrap(window.ReportBuilderV1,'render','__currentExecutiveConclusionV7');wrap(window.ReportBuilderV8,'apply','__currentExecutiveConclusionV7');wrap(window.ReportBuilderV8Presentation,'apply','__currentExecutiveConclusionV7');wrap(window.PropertyThesisReportBranding,'apply','__currentExecutiveConclusionV7');apply();}
  document.addEventListener('click',e=>{if(e.target?.closest?.('[data-s8-tab="report"],[data-tab="report"],#rbRefresh,[data-hub-report],[data-pt-report]'))setTimeout(apply,120);},true);
