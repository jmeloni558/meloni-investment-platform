...
(() => {
  ...
  function wireStepTwo(){const step=document.querySelector('[data-s8-tab="assumptions"]');if(step&&!step.dataset.propertySyncV15){step.dataset.propertySyncV15='1';step.addEventListener('click',()=>{syncPropertyToAnalysis(true);setTimeout(()=>syncPropertyToAnalysis(true),0)},true)}}

  function loadStage13(){
    document.getElementById('stage13Script')?.remove();
    const s=document.createElement('script');s.id='stage13Script';s.src='stage13.js?v=6';document.body.appendChild(s);
  }
