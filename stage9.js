...
(() => {
  ...
  function wireStepTwo(){const step=document.querySelector('[data-s8-tab="assumptions"]');if(step&&!step.dataset.propertySyncV11){step.dataset.propertySyncV11='1';step.addEventListener('click',()=>{syncPropertyToAnalysis(true);setTimeout(()=>syncPropertyToAnalysis(true),0)},true)}}

  function loadStage13(){
    const old=document.getElementById('stage13Script');if(old)old.remove();
    const s=document.createElement('script');s.id='stage13Script';s.src='stage13.js?v=2';document.body.appendChild(s);
  }
