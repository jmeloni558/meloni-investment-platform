...
(() => {
  ...
  function wireStepTwo(){const step=document.querySelector('[data-s8-tab="assumptions"]');if(step&&!step.dataset.propertySyncV9){step.dataset.propertySyncV9='1';step.addEventListener('click',()=>{syncPropertyToAnalysis(true);setTimeout(()=>syncPropertyToAnalysis(true),0)},true)}}

  function loadStage11(){
    const old=document.getElementById('stage11Script');if(old)old.remove();
    const s=document.createElement('script');s.id='stage11Script';s.src='stage11.js?v=5';document.body.appendChild(s);
  }
