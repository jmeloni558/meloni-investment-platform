...
(()=>{
  ...
  function apply(){
    ...
    head.innerHTML=`<div class="pt-split-head"><div class="pt-user-side" style="border-bottom:4px solid ${safeColor(p.brand_color)}">${p.logo_url?`<img src="${esc(p.logo_url)}" alt="${esc(company)} logo">`:''}<div><span class="pt-user-label">Presented by</span><strong class="pt-user-company">${esc(company)}</strong>${person?`<span class="pt-user-person">${esc(person)}</span>`:''}${contact?`<span class="pt-user-contact">${esc(contact)}</span>`:''}</div></div><div class="pt-product-side"><div class="pt-product-name">${PRODUCT}</div><div class="pt-product-tag">${TAGLINE}</div><div class="pt-product-mark"></div></div></div><div class="pt-title-band"><h1>${TYPE}</h1><p class="address">${esc(address)}</p>${metaHtml?`<div class="rb-meta">${metaHtml}</div>`:''}</div>`;
