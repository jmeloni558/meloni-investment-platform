...
(()=>{
  ...
  function query(){
    ...
    return{address:raw('address'),radius:raw('radius'),city:raw('city'),state:raw('state').toUpperCase(),zipCode:raw('zipCode'),...filters,propertyTypes,sort:raw('sort'),limit:18,offset};
  }
  async function search(){
    offset=0;const q=query();if(!q.address&&!q.zipCode&&!(q.city&&q.state)){setStatus('Enter a starting address, a ZIP code, or both a city and two-letter state code.');return;}if(!q.propertyTypes.length){setStatus('Select at least one property type.');return;}setStatus('Searching active listings…');panel.querySelector('.pt-listings-grid').innerHTML='';
