'use strict';
(()=>{
  const VERSION=9,IMPORT_KEY='ptPendingListingImportV1';
  ...
  function updateUsage(usage,cached=false){const box=panel?.querySelector('.pt-listings-usage');if(!box||!usage)return;const free=usage.plan==='free';const warn=free&&Number(usage.dailyRemaining)<=2;if(!warn){box.hidden=true;return;}box.hidden=false;box.querySelector('[data-usage-title]').textContent=Number(usage.dailyRemaining)>0?`${usage.dailyRemaining} free listing search${Number(usage.dailyRemaining)===1?'':'es'} remaining today`:'Free listing allowance used for today';box.querySelector('[data-usage-copy]').textContent=cached?'This result came from cache and did not use another call. Upgrade for a larger daily discovery allowance.':'Upgrade for a larger daily discovery allowance and continued property research.';}
  ...
  const capRates=[.05,.055,.06,.065,.07];
  ...
  function renderValueView(modal,listing,model,error=''){
    ...
    const price=Number(listing.price)||0,annualGross=model.grossMonthly*12,noi=annualGross*.60,currentCap=price>0?noi/price:null;
    ...
    root.innerHTML=`<div class="pt-value-head"><div><span>PROPERTYTHESIS VALUE VIEW</span><h4>What does the income support?</h4></div><span class="pt-rent-source">${esc(sourceNote)}</span></div><div class="pt-value-summary"><div><span>Monthly gross rent</span><b>${money(model.grossMonthly)}</b></div><div><span>Estimated NOI</span><b>${money(noi)}</b></div><div class="is-primary"><span>Cap rate at list price</span><b>${pct(currentCap)}</b></div></div><label class="pt-value-rent-input"><span>Adjust monthly gross rent</span><input type="number" min="0" step="25" value="${Math.round(model.grossMonthly)}" data-value-rent><em>Press Enter or leave the field to update</em></label><div class="pt-value-ladder"><div class="pt-value-ladder-head"><b>Cap-rate benchmark</b><b>Income-supported value</b></div>${capRates.map(rate=>`<div class="${rate===.065?'is-benchmark':''}"><span>${(rate*100).toFixed(1)}%${rate===.065?' · benchmark':''}</span><b>${money(noi/rate)}</b></div>`).join('')}</div><div class="pt-value-assumption"><b>Quick-screen assumption:</b> Operating costs equal 40% of gross rent, leaving 60% as estimated NOI. ${esc(range)}This is a preliminary income screen, not a substitute for verified leases, expenses, vacancy, reserves, capital costs, or complete underwriting.</div>`;
    bindValueInput(root,listing,model);
  }
  function bindValueInput(root,listing,model){const input=root.querySelector('[data-value-rent]');const apply=()=>{const gross=Number(input?.value);if(!Number.isFinite(gross)||gross<0)return;renderValueView(root.closest('.pt-listing-detail'),listing,{...(model||rentModel(listing,gross,'User-entered rent')),grossMonthly:gross,source:'User-entered rent',multi:false});};input?.addEventListener('change',apply);input?.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();apply();}});}
  ...
  function detail(l){
    ...
    bindStreetViews(modal);modal.querySelector('.pt-listings-close').onclick=()=>modal.classList.remove('is-open');modal.querySelector('[data-import]').onclick=()=>importListing(activeListing);modal.classList.add('is-open');renderValueView(modal,l,null);loadFeatureFacts(l,modal);loadValueView(l,modal);
  }
