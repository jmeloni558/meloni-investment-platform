...
(() => {
  ...
  function injectGuidanceStyles(){
    ...
    const st=document.createElement('style');st.id='stage11GuidanceStyles';
  ...
  function applyRentGrowthGuidance(){
    ...
    if(!field.querySelector('.guidance-box[data-guide="rentGrowth"]')){
      const box=document.createElement('div');box.className='guidance-box';box.dataset.guide='rentGrowth';
  ...
  function apply(){
    const sec=document.getElementById('assumptions');if(!sec)return false;
    const original=document.getElementById('calculateBtn');
    const head=original?.closest('.sectionhead')||sec.querySelector('.sectionhead');if(!head)return false;
    ...
    if(actions&&!document.getElementById('guidedReviewResults')){
    ...
    if(!bottom){
      bottom=document.createElement('div');bottom.id='guidedReviewResultsBottom';bottom.className='card span-12 screen-only';
    ...
    document.getElementById('guidedReviewResultsBottomBtn').onclick=reviewResults;
  ...
  function start(){
    ...
    const timer=setInterval(()=>{
      const ready=apply();applyFreshStartup();
      if((ready&&startupApplied)||++tries>40)clearInterval(timer);
    },125);
