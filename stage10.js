...
(() => {
  ...
  function toggleAdvanced(){
    ...
    const btn=document.getElementById('s8AdvancedToggle');
  ...
  function rebuildWorkflow(){
    ...
    steps.innerHTML=`
      <div class="s10-row-label"><span>Primary Workflow</span><small>Complete these steps in order for a typical analysis</small></div>
      <div class="s10-step-grid">
    ...
    if(!utilities){
      ...
      utilities.id='s10Utilities';
      utilities.className='s10-utilities';
      steps.insertAdjacentElement('afterend',utilities);
    }
    utilities.innerHTML=`
      <div class="s10-utilities-label"><span>Utilities</span><small>Use these only when you need saved files or detailed tools</small></div>
      <div class="s10-utilities-actions">
        ...
        <button class="s8-advanced-toggle s10-advanced" id="s8AdvancedToggle" aria-expanded="false"><span class="s10-tool-icon">⚙</span><span><b>Advanced Tools</b><small>Schedules, scenarios & cloud tools</small></span></button>
      </div>`;
    ...
    st.textContent=`
      #stage8Workflow .s10-workflow-row{display:block;border:1px solid var(--line);border-top:0;border-radius:0;background:#fff;overflow:visible}
      .s10-row-label{display:flex;justify-content:space-between;gap:12px;align-items:center;padding:8px 12px;background:#f7fbff;border-bottom:1px solid #d9e7f2}
      .s10-row-label span,.s10-utilities-label span{font-size:10px;font-weight:900;text-transform:uppercase;letter-spacing:.07em;color:#174f83}
      .s10-row-label small,.s10-utilities-label small{font-size:9px;color:#667085}
      .s10-step-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr))}
      .s10-step-grid .s8-step{min-height:62px;border-right:1px solid var(--line);border-bottom:0}
      .s10-step-grid .s8-step:last-child{border-right:0}
      .s10-utilities{margin-top:10px;border:1px solid #d8dee6;border-radius:10px;background:#f8fafc;overflow:hidden}
      .s10-utilities-label{display:flex;justify-content:space-between;gap:12px;align-items:center;padding:7px 12px;border-bottom:1px solid #e3e8ee;background:#f1f4f7}
      .s10-utilities-label span{color:#596579}
      .s10-utilities-actions{display:flex;gap:8px;padding:9px 10px}
      .s10-existing,.s10-advanced{appearance:none;border:1px solid #d8dee6;border-radius:8px;background:#fff;padding:9px 12px;display:flex;gap:9px;text-align:left;align-items:center;cursor:pointer;min-width:210px}
      .s10-existing:hover,.s10-advanced:hover{background:#f4f7fa;border-color:#bfc9d4}
      .s10-existing.active,.s10-advanced.active{background:#eef3f7;border-color:#9fb3c5}
      .s10-existing b,.s10-advanced b{display:block;font-size:11px;color:#344054}
      .s10-existing small,.s10-advanced small{display:block;font-size:9px;line-height:1.3;color:#667085;margin-top:2px}
  ...
  function wireControls(){
    document.querySelectorAll('#stage8Workflow [data-s8-tab]').forEach(b=>b.onclick=()=>switchTab(b.dataset.s8Tab));
  ...
  function refreshState(){
    ...
    if(next){
      ...
      else if(advancedTabs.includes(active))next.textContent='Advanced utility view: use this detailed tool, then return to the primary workflow.';
    }
  }
