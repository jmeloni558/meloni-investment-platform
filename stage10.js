...
(() => {
  ...
  function rebuildWorkflow(){
    ...
    let utilities=document.getElementById('s10Utilities');
    if(!utilities){utilities=document.createElement('div');utilities.id='s10Utilities';utilities.className='s10-utilities';steps.insertAdjacentElement('afterend',utilities)}
    utilities.innerHTML=`<div class="s10-utilities-label"><span>Utilities</span><small>Use these only when you need saved files or detailed tools</small></div><div class="s10-utilities-actions"><button class="s10-existing" id="s10Existing"><span class="s10-icon">⌕</span><span><b>Existing Properties</b><small>Search and reopen saved files</small></span></button><button class="s8-advanced-toggle s10-advanced" id="s8AdvancedToggle"><span class="s10-tool-icon">⚙</span><span><b>Advanced Tools</b><small>Schedules, scenarios & cloud tools</small></span></button></div>`;

    const advancedPanel=document.getElementById('s8AdvancedPanel');if(advancedPanel&&advancedPanel.parentElement!==utilities)utilities.appendChild(advancedPanel);
