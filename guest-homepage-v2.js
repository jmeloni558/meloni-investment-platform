...
(()=>{
  ...
  function upgradeHero(root){
    ...
    if(title)title.textContent='Know the Numbers. Make the Offer.';
    if(lead)lead.textContent='A property thesis is the idea that a rental property can produce the income, value and returns you expect. PropertyThesis tests that investment case against the numbers—so you can see whether the evidence supports it.';
    if(sub)sub.textContent='Analyze rental income, operating expenses, financing, cash flow, taxes, property value, investor returns and market evidence in one guided workflow, then create a client-ready investment report.';
    let pathway=root.querySelector('.pt-home-thesis-path');if(!pathway){pathway=document.createElement('div');pathway.className='pt-home-thesis-path';sub?.insertAdjacentElement('afterend',pathway);}pathway.innerHTML='<span><b>01</b>Form the idea</span><i>→</i><span><b>02</b>Test the assumptions</span><i>→</i><span><b>03</b>Make the offer</span>';
    let audience=root.querySelector('.pt-home-audience');if(!audience){audience=document.createElement('p');audience.className='pt-home-audience';}pathway.insertAdjacentElement('afterend',audience);audience.textContent='Real estate investment analysis software for investors, agents, brokers and acquisition professionals who need more than a basic rental property calculator.';
