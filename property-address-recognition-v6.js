...
(()=>{
  ...
  function syncAddress(value){
    ...
    try{
      ...
      queueMicrotask(()=>{syncingAddress=false;});
    }
  }
  ...
  async function lookup(address){
    ...
    try{
      ...
      setTimeout(()=>{hideSuggestions();focusNextField();},0);
      setTimeout(()=>hideSuggestions(),80);
    }catch(e){
  ...
  async function attachInput(input){
    ...
    try{
      ...
      ac.addListener('place_changed',()=>{
        ...
        syncAddress(address);
        setTimeout(()=>hideSuggestions(input),0);
        lookup(address);
      });
      input.addEventListener('input',()=>{if(!syncingAddress)showSuggestions();},{passive:true});
      input.addEventListener('focus',()=>{if(!syncingAddress&&input.value.trim()!==lastLookup)showSuggestions();},{passive:true});
