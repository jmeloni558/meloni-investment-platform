...
(() => {
  ...
  function branding() {
    ...
    const brand = (cover?.querySelector('.rb-brand')?.textContent || '').trim() || 'PropertyThesis';
    ...
    const who = (prepared?.textContent || '').replace(/^\s*Prepared by:\s*/i, '').trim() || 'Report Author';
    const contact = (cover?.querySelector('.rb-profile-contact')?.textContent || '').replace(/^\s*Contact:\s*/i, '').trim();
    const address = (window.state?.address || window.state?.name || document.querySelector('#clientReport .rb-cover .address')?.textContent || 'Income-Producing Property').trim();
    return {brand, logo, who, contact, address};
  }

  function stateObj(){ return window.state || {}; }
  function resultObj(){ return window.result || {}; }
  function years(){ return resultObj().years || []; }
  ...
  function brandRows(title) {
    ...
    return `<tr class="brand"><td colspan="80"><b>${esc(b.brand)}</b></td></tr>${logo}<tr class="title"><td colspan="80"><b>${esc(title)}</b></td></tr><tr><td colspan="80">${esc(b.address)}</td></tr><tr><td colspan="80">Prepared by: ${esc(b.who)}</td></tr>${b.contact ? `<tr><td colspan="80">Contact: ${esc(b.contact)}</td></tr>` : ''}<tr><td colspan="80">Generated by PropertyThesis</td></tr><tr><td colspan="80"></td></tr>`;
  }
  ...
  function schedule() { [0, 60, 150, 300, 700, 1200, 2000, 3500].forEach(ms => setTimeout(forceButton, ms)); }
  function watch() {
  ...
  window.PropertyThesisProFormaExcelFinal = {forceButton, downloadExcel};
