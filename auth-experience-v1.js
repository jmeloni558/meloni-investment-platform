...
(()=>{
  ...
  function ensureUi(){
    ...
    if(!el('ptAuthVerification')){const verification=document.createElement('div');verification.id='ptAuthVerification';verification.hidden=true;verification.innerHTML='<h3>Check your email to finish</h3><p>We sent a verification link to <span class="pt-auth-email" id="ptAuthVerificationEmail"></span>.</p><p><strong>Do not try to sign in yet.</strong> Open the verification email and click its link. PropertyThesis will return you to the current site and sign you in automatically.</p><p id="ptAuthPendingCopy">If you started an analysis, your assumptions will be restored, calculated, and saved after verification.</p><div class="actions"><button class="btn secondary" id="resendConfirmationAction" type="button">Resend Verification Email</button><button class="btn ghost" id="returnSignInAction" type="button">I Already Verified — Sign In</button></div>';modes.insertAdjacentElement('afterend',verification);el('returnSignInAction').onclick=()=>setMode('signin');}
    return true;
  }
  ...
  function showVerification(email){
    ...
    ['authEmail','authPassword'].forEach(id=>el(id)?.closest('.field')?.classList.add('pt-auth-mode-hidden'));el('ptAuthConfirmWrap')?.classList.add('pt-auth-mode-hidden');el('ptAuthPasswordHint')?.classList.add('pt-auth-mode-hidden');el('signInAction')?.closest('.actions')?.classList.add('pt-auth-mode-hidden');message('Verification email sent. Complete verification before signing in.','success');
  }
  function restoreForm(){el('ptAuthModes')?.classList.remove('pt-auth-mode-hidden');['authEmail','authPassword'].forEach(id=>el(id)?.closest('.field')?.classList.remove('pt-auth-mode-hidden'));el('signInAction')?.closest('.actions')?.classList.remove('pt-auth-mode-hidden');}
