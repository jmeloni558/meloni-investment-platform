...
(() => {
  ...
  function renderPropertyFile(){
    ...
    if(!p){body.innerHTML='<div class="card span-12 pf-empty">Select a property from Property Workspace.</div>';return}
    ...
    body.innerHTML=`
      <div class="card span-12">
        <div class="pf-breadcrumb"><button id="pfBack">Property Workspace</button><span>›</span><span>${esc4(p.name||'Property File')}</span></div>
        <div class="pf-hero"><div><h2>${esc4(p.name||'Untitled Property')}</h2><p>${esc4(p.address||[p.city,p.state,p.postal_code].filter(Boolean).join(', ')||'No address entered')}</p></div><div class="actions"><span class="pf-status">${p.archived?'Archived':statusFor(a)}</span><button class="btn primary" id="pfOpenAnalysis">Analysis Dashboard</button><button class="btn secondary" id="pfReport">Generate Report</button><button class="btn ghost" id="pfNewAnalysis">New Analysis</button></div></div>
      </div>
      ...
      <div class="span-12 pf-two"><div class="card"><h2>Property & Client</h2><div class="pf-line"><span>Client</span><b>${esc4(c?.name||'Unassigned')}</b></div><div class="pf-line"><span>Email</span><b>${esc4(c?.email||'—')}</b></div><div class="pf-line"><span>Phone</span><b>${esc4(c?.phone||'—')}</b></div><div class="pf-line"><span>Property status</span><b>${p.archived?'Archived':'Active'}</b></div><div class="pf-line"><span>Property updated</span><b>${safeDate(p.updated_at)}</b></div><div class="pf-line"><span>Notes</span><b>${esc4(p.notes||'—')}</b></div><div class="actions" style="margin-top:12px"><button class="btn ghost" id="pfEditProperty">Edit Property Record</button><button class="btn ${p.archived?'secondary':'danger'}" id="pfArchive">${p.archived?'Restore Property':'Archive Property'}</button></div></div>
  ...
  function wireFileActions(){
    ...
    document.getElementById('pfOpenAnalysis').onclick=()=>latest?loadAnalysisVersion(latest.id):newAnalysisForProperty();
  ...
  function enhanceWorkspace(){
    document.querySelectorAll('#hubCards .hub-card').forEach(card=>{const open=card.querySelector('[data-hub-open]');if(!open)return;const pid=open.dataset.hubOpen;if(!pid||card.querySelector('[data-stage7-file]'))return;const b=document.createElement('button');b.className='btn primary';b.dataset.stage7File=pid;b.textContent='Open Property File';b.onclick=()=>openPropertyFile(pid);open.classList.remove('primary');open.classList.add('ghost');open.textContent='Analysis Dashboard';open.parentElement?.insertBefore(b,open)});
  }
