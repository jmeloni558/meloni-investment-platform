...
(()=>{
  ...
  function endingBalance(y){
    try{if(!finite(state?.mortgage)||n(state.mortgage)<=0)return 0;const paid=years().filter(x=>x.year<=y.year).reduce((s,x)=>s+(finite(x.principal)?n(x.principal):0),0);return Math.max(0,n(state.mortgage)-paid);}catch(_e){return NaN;}
  }
  ...
  function injectControls(){
    ...
    if(toggles&&!document.getElementById('ptIncludeProForma')){
      const label=document.createElement('label');label.className='rb-toggle';
      label.innerHTML=`<input type="checkbox" id="ptIncludeProForma" ${loadPref()?'checked':''}><span>5-Year Pro Forma</span>`;
      toggles.appendChild(label);
      label.querySelector('input')?.addEventListener('change',e=>{savePref(e.target.checked);applyReport();});
    }
    const actions=controls.querySelector('.rb-actions');
    if(actions&&!document.getElementById('rbDownloadProForma')){
      const b=document.createElement('button');b.type='button';b.id='rbDownloadProForma';b.className='btn secondary';b.textContent='Download Pro Forma Excel';
  ...
  function applyCurrencyFormat(ws,range){
    const XLSX=window.XLSX;if(!XLSX||!range)return;const r=XLSX.utils.decode_range(range);
    for(let R=r.s.r;R<=r.e.r;R++)for(let C=r.s.c;C<=r.e.c;C++){const cell=ws[XLSX.utils.encode_cell({r:R,c:C})];if(cell&&cell.t==='n')cell.z='$#,##0;[Red]($#,##0);-';}
  }
  function styleWorksheet(ws,widths){ws['!cols']=widths.map(w=>({wch:w}));}

  function operatingSheet(XLSX){
    const ys=years(),rows=proFormaRows(ys),aoa=sheetTitleRows('PropertyThesis Operating Pro Forma');
    aoa.push(['Metric',...ys.map(y=>'Year '+y.year)]);
    rows.forEach(r=>aoa.push([r.label,...r.values.map(v=>finite(v)?n(v):'')]));
    const ws=XLSX.utils.aoa_to_sheet(aoa);styleWorksheet(ws,[34,...ys.map(()=>15)]);applyCurrencyFormat(ws,`B8:${XLSX.utils.encode_col(ys.length)}${7+rows.length}`);return ws;
  }
  function taxesSheet(XLSX){
    const ys=years(),aoa=sheetTitleRows('PropertyThesis Taxes From Operations');
    aoa.push(['Year','NOI','Interest','Depreciation','Points Amortization','Origination Fee Amortization','Taxable Income','Taxes From Operations','After-Tax Operating Cash Flow']);
    ys.forEach(y=>aoa.push([y.year,y.noi,y.interest,result?.depreciation,pointsAmort(y),origAmort(y),y.taxable,y.opTax,y.atcf].map((v,i)=>i===0?v:(finite(v)?n(v):''))));
    const ws=XLSX.utils.aoa_to_sheet(aoa);styleWorksheet(ws,[10,16,16,16,20,22,17,20,23]);applyCurrencyFormat(ws,`B8:I${7+ys.length}`);return ws;
  }
  function dispositionSheet(XLSX){
    const hold=Math.max(1,Math.round(n(state?.hold)||1));
    const gainRate=hold===1?n(state?.ordinaryTax||0):n(state?.capGainsTax||0);
    const netSale=finite(result?.netSale)?n(result.netSale):NaN;
    const sellCost=finite(state?.sellCost)?n(state.sellCost):NaN;
    const grossSale=finite(netSale)&&finite(sellCost)&&sellCost<1?netSale/(1-sellCost):NaN;
    const sellingExpenses=finite(grossSale)&&finite(netSale)?grossSale-netSale:NaN;
    ...
    const aoa=sheetTitleRows('PropertyThesis Disposition & Taxes on Sale');
    aoa.push(['Disposition Metric','Value']);
    [
      ['Year of Sale',hold],['Projected Gross Sale Price',grossSale],['Selling Expenses',sellingExpenses],['Net Sales Price',result?.netSale],['Loan Payoff',result?.loanPayoff],['Book Value',result?.book],['Gain / Loss on Sale',result?.gain],['Applicable Gain Tax Rate',gainRate],['Taxes Due on Gain / Loss',taxesGain],['Accumulated Depreciation',result?.accDep],['Depreciation Tax Rate',state?.depTax],['Taxes Due on Depreciation',depTax],['Total Taxes Due on Sale',result?.saleTax],['After-Tax Equity Reversion',result?.ater]
    ].forEach(r=>aoa.push(r.map((v,i)=>i===0?v:(finite(v)?n(v):v??''))));
    const ws=XLSX.utils.aoa_to_sheet(aoa);styleWorksheet(ws,[34,20]);
    for(let R=7;R<aoa.length;R++){const label=aoa[R]?.[0]||'',cell=ws[`B${R+1}`];if(!cell||cell.t!=='n')continue;if(/Rate/.test(label))cell.z='0.00%;[Red](0.00%);-';else if(label!=='Year of Sale')cell.z='$#,##0;[Red]($#,##0);-';}
    return ws;
  }
  ...
  async function downloadExcel(){
    ...
    try{
      ...
      XLSX.utils.book_append_sheet(wb,operatingSheet(XLSX),'Operating Pro Forma');
      XLSX.utils.book_append_sheet(wb,taxesSheet(XLSX),'Taxes From Operations');
      XLSX.utils.book_append_sheet(wb,dispositionSheet(XLSX),'Disposition & Sale Tax');
  ...
  function schedule(){[0,80,220].forEach(ms=>setTimeout(()=>{injectControls();applyReport();},ms));}
  function wrapReport(){const api=window.ReportBuilderV1,fn=api?.renderReport;if(typeof fn!=='function'||fn.__ptProFormaWrapped)return;const wrapped=function(){const out=fn.apply(this,arguments);setTimeout(()=>{injectControls();applyReport();},0);return out;};wrapped.__ptProFormaWrapped=true;wrapped.__original=fn;api.renderReport=wrapped;if(api.render===fn)api.render=wrapped;}
  function start(){wrapReport();schedule();document.addEventListener('click',e=>{
    if(e.target?.closest?.('#rbDownloadProForma')){e.preventDefault();e.stopPropagation();downloadExcel();return;}
    if(e.target?.closest?.('#rbSelectCore,#rbSelectAll')){savePref(true);setTimeout(()=>{const x=document.getElementById('ptIncludeProForma');if(x)x.checked=true;applyReport();},20);}
