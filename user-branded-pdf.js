...
(() => {
  ...
  const CAPTURE_WIDTH=816,FOOTER_H=48,TOP_PAD=24,BOTTOM_PAD=10,PAGE_GAP=10;
  ...
  async function preparePreview(){window.ReportBuilderV1?.renderReport?.();await new Promise(r=>setTimeout(r,180));window.ReportBuilderV8?.apply?.();window.UserBranding?.applyReportBranding?.();window.PropertyThesisReportBranding?.apply?.();await new Promise(r=>setTimeout(r,220));}
  ...
  function statRows(section){const stats=[...section.querySelectorAll(':scope .rb-stat')].map(el=>({el,r:el.getBoundingClientRect()})).sort((a,b)=>a.r.top-b.r.top||a.r.left-b.r.left);const rows=[];let row=[];for(const item of stats){if(!row.length||Math.abs(item.r.top-row[0].r.top)<8)row.push(item);else{rows.push(row.map(x=>x.el));row=[item];}}if(row.length)rows.push(row.map(x=>x.el));return rows;}
  ...
  function items(report){const out=[];const add=(kind,els,gap=10,keepNext=false)=>{const arr=(Array.isArray(els)?els:[els]).filter(Boolean);if(arr.length)out.push({kind,els:arr,gap,keepNext});};
    ...
    for(const section of report.querySelectorAll(':scope > .rb-section')){
      ...
      for(const panel of section.querySelectorAll(':scope > .rb-panel'))add('full',panel,10);
      for(const table of section.querySelectorAll(':scope > .rb-tablewrap'))add('full',table,10);
    }
    return out;
  }
  ...
  async function render(doc,report,list){const pageW=doc.internal.pageSize.getWidth(),pageH=doc.internal.pageSize.getHeight(),reportW=report.getBoundingClientRect().width,scalePt=pageW/reportW,bodyBottom=pageH-FOOTER_H-BOTTOM_PAD;let page=1,y=0;
    const newPage=()=>{doc.addPage();page++;y=TOP_PAD;};
    for(let i=0;i<list.length;i++){
      const it=list[i];
      if(it.kind==='row'){
        const rects=it.els.map(el=>el.getBoundingClientRect());const rowTop=Math.min(...rects.map(r=>r.top)),rowBottom=Math.max(...rects.map(r=>r.bottom));const hPt=(rowBottom-rowTop)*scalePt,gapPt=it.gap;
        ...
        for(let j=0;j<it.els.length;j++){const el=it.els[j],r=rects[j],c=await snap(el);if(!c)continue;const x=(r.left-report.getBoundingClientRect().left)*scalePt,w=r.width*scalePt,h=r.height*scalePt;doc.addImage(c.toDataURL('image/jpeg',0.98),'JPEG',x,y+(r.top-rowTop)*scalePt,w,h,undefined,'FAST');}
        y+=hPt;continue;
      }
      const el=it.els[0],r=el.getBoundingClientRect(),wPt=r.width*scalePt,hPt=r.height*scalePt,xPt=(r.left-report.getBoundingClientRect().left)*scalePt,gapPt=it.gap;
      let nextExtra=0;if(it.keepNext&&list[i+1]){const nr=list[i+1].els[0].getBoundingClientRect();nextExtra=(nr.height*scalePt)+PAGE_GAP;}
      ...
      if(hPt<=fullPageCapacity){if(y+gapPt+hPt+nextExtra>bodyBottom&&y>TOP_PAD+4)newPage();y+=gapPt;const c=await snap(el);if(c)doc.addImage(c.toDataURL('image/jpeg',0.98),'JPEG',xPt,y,wPt,hPt,undefined,'FAST');y+=hPt;}
        ...
        // Only oversized single elements are split, using their own clean canvas rather than a master report screenshot.
