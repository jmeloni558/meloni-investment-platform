...
(() => {
  ...
  function ensureCaptureStyles(){
    ...
    s.textContent=`
      ...
      #clientReport .pt-pdf-capture .rb-tablewrap{width:100%!important;max-width:100%!important;overflow:visible!important}
      #clientReport .pt-pdf-capture table{width:100%!important;max-width:100%!important;min-width:0!important;table-layout:fixed!important;border-collapse:collapse!important}
      #clientReport .pt-pdf-capture th,#clientReport .pt-pdf-capture td{min-width:0!important;width:auto!important;white-space:normal!important;overflow:visible!important;text-overflow:clip!important;overflow-wrap:anywhere!important;padding:6px 5px!important;font-size:9px!important;line-height:1.25!important}
      #clientReport .pt-pdf-capture .rb-row,#clientReport .pt-pdf-capture .rb-stat,#clientReport .pt-pdf-capture .rb-panel,#clientReport .pt-pdf-capture .rb-analysis-copy,#clientReport .pt-pdf-capture .rb-conclusion-box{break-inside:avoid!important}
    `;
  }
  ...
  function naturalBreaks(report,scale,total){
    ...
    const topLevel=[...report.querySelectorAll(':scope > .rb-cover,:scope > .rb-conclusion,:scope > .rb-findings,:scope > .rb-section,:scope > .rb-footer')];
    for(const el of topLevel){addCandidate(set,relY(el,'top'),total);addCandidate(set,relY(el,'bottom'),total);}

    const internals=[...report.querySelectorAll('.rb-analysis-summary .rb-section-head,.rb-summary-block,.rb-conclusion-box,.rb-analysis-copy p,.rb-panel,.rb-final-copy p,.rb-tablewrap thead,.rb-tablewrap tbody tr')];
    for(const el of internals)addCandidate(set,relY(el,'bottom')+2*scale,total);

    // Metric cards are usually arranged in rows. Add one safe break after each visual row.
    ...
    let group=[];
    const flush=()=>{if(!group.length)return;const bottom=Math.max(...group.map(x=>x.r.bottom));addCandidate(set,(bottom-root.top)*scale+2*scale,total);group=[];};
  ...
  function chooseBreak(start,maxEnd,breaks,total){
    ...
    const preferredMin=start+capacity*0.72;
    const acceptableMin=start+capacity*0.50;
  ...
  async function generate(){const btn=document.getElementById('rbDownloadPdf');if(btn){btn.disabled=true;btn.textContent='Generating PDF...';}let clone=null;try{
      ...
      clone=makeCaptureClone(source);await new Promise(r=>setTimeout(r,220));
      ...
      while(start<canvas.height-1){
        ...
        const imgH=h/pxPerPt;doc.addImage(slice.toDataURL('image/jpeg',0.97),'JPEG',0,0,pageW,imgH,undefined,'FAST');
        start=end;page++;
      }
      doc.save(filename());status('PDF generated with optimized pagination');
