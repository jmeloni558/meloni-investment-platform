...
- **14.4 PASS:** The Supabase publishable key is protected by RLS. Google Cloud Console confirms the Maps browser key is restricted to `https://propertythesis.com/*` and `https://www.propertythesis.com/*`, and API access is limited to Maps JavaScript API, Places API, Places API (New), and Street View Static API.
...
- **14.6 PASS (policy + implementation verification):** Archive is an owner-scoped reversible flag hidden from the default property dashboard and available through “Show archived.” Permanent deletion requires explicit confirmation, is owner-scoped, verifies that a row was deleted, refreshes the library, and cascades through analyses, scenarios, analysis versions, and entitlements by database constraints.
...
- Supabase's security advisor reports leaked-password protection is disabled. Dashboard inspection confirmed this control requires Supabase Pro; it cannot be enabled on the current Free plan. Treat enabling it as a future paid-plan hardening item rather than a current configuration defect.
