'use strict';
(() => {
  if(window.__reviewValuationVersion>=1)return;
  window.__reviewValuationVersion=1;
  ...
  function injectStyles(){
    ...
    st.textContent=`
      ...
      #dashboard .review-old-valuation-hidden{display:none!important}
    `;
  ...
  function ensureValueBox(field,id,label,explanation){
    if(!field)return null;
    let box=field.querySelector('#'+id);
    if(!box){
      box=document.createElement('div');
      box.id=id;
      box.className='review-implied-value';
