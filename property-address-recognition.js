...
(()=>{
  ...
  function currentAddress(input){return String(input?.value||document.getElementById('f_address')?.value||getState()?.address||'').trim();}
  function syncAddress(address){addressInputs().forEach(el=>{if(el.value!==address){el.value=address;el.dispatchEvent(new Event('input',{bubbles:true}));el.dispatchEvent(new Event('change',{bubbles:true}));}});const s=getState();if(s)s.address=address;}
  ...
  async function lookup(input){if(busy)return;const address=currentAddress(input);if(!address){renderAll('error','Enter the full street address, city, state and ZIP first.');input?.focus();return;}try{if(typeof cloudUser==='undefined'||!cloudUser){renderAll('error','Sign in to PropertyThesis before recognizing a property.');return;}if(typeof cloudClient==='undefined'||!cloudClient){renderAll('error','Property lookup is not available yet.');return;}}catch(_e){renderAll('error','Property lookup is not available yet.');return;}
    ...
    try{const {data,error}=await cloudClient.functions.invoke('rentcast-property-lookup',{body:{address}});if(error)throw error;if(data?.error)throw new Error(data.error);if(!data?.matched||!data?.property){renderAll('error','No matching RentCast property record was found. You can continue with the address manually.');return;}const p=data.property;syncAddress(p.formattedAddress||address);const s=getState();if(s)s.subjectProperty={...(s.subjectProperty||{}),...p,recognizedAt:new Date().toISOString(),source:'RentCast'};markDirty();renderAll('ok',summary(p));try{if(typeof setStatus==='function')setStatus('Property recognized');}catch(_e){}
