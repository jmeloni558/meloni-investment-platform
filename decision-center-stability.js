...
(()=>{
  ...
  function styles(){
    if(document.getElementById('ptDecisionCenterOrderStyles'))return;
    const s=document.createElement('style');s.id='ptDecisionCenterOrderStyles';s.textContent=`
      #dashboard .grid>#reviewAnalysisSetup{order:-100!important}
      #dashboard .grid>#ptDecisionCenter{order:-99!important}
      #dashboard .grid>#ptDecisionCenter.ptdc-loading{display:none!important}
  ...
  function markLoading(on){
    const card=document.getElementById('ptDecisionCenter');
    if(card)card.classList.toggle('ptdc-loading',!!on);
  }

  function install(){
    styles();
    const api=window.PropertyThesisDecisionCenter;
    if(!api||api.__ptStableV2)return false;
    ...
    api.apply=function(){
      ...
      if(!matchesCurrent(offer)){
        ...
        Promise.resolve(window.PropertyThesisSecondaryEngine?.request?.({refresh:false})).then(()=>{
          const fresh=window.PropertyThesisSecondaryEngine?.getOffer?.();
          if(!matchesCurrent(fresh))return;
          try{oldApply?.();markLoading(false);pin();}catch(_e){}
        });
        return false;
      }
      const out=oldApply?.();markLoading(false);pin();return out;
    };
    api.pin=pin;
    api.__ptStable=true;
    api.__ptStableV2=true;
    return true;
  }
  ...
  function refresh(){
    install();
    const offer=window.PropertyThesisSecondaryEngine?.getOffer?.();
    if(!matchesCurrent(offer))markLoading(true);
    try{window.PropertyThesisDecisionCenter?.apply?.();}catch(_e){}
    pin();
  }

  function observe(){
    const grid=document.querySelector('#dashboard .grid');if(!grid||grid.__ptDecisionPinObservedV2)return false;
    grid.__ptDecisionPinObservedV2=true;
