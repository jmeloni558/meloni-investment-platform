...
(()=>{
  ...
  async function currentPreview(){
    ...
    // Critical: do NOT rerender or reapply thesis/report modules here. The PDF must
    // capture the exact report DOM the user is currently reviewing.
    await sleep(80);
    return report;
  }
  ...
  function makeStage(){const host=document.getElementById('clientReport');if(!host)throw new Error('Client report container is unavailable.');const stage=document.createElement('div');stage.className='pt-pdf-stage-v22';Object.assign(stage.style,{position:'fixed',left:'-12000px',top:'0',width:CAPTURE_WIDTH+'px',maxWidth:CAPTURE_WIDTH+'px',height:'auto',zIndex:'-10',background:'#fff',overflow:'visible'});host.appendChild(stage);return stage;}

  function nodeBlock(node,kind='block',label='Report block',keepNext=false){return {type:'node',node,kind,label,keepNext};}
  function rowBlock(nodes,label='Stat row'){return {type:'row',nodes,label,kind:'section-child',keepNext:false};}

  function statRows(container,title){
    const stats=[...container.querySelectorAll(':scope > .rb-stat')];
    if(!stats.length)return [nodeBlock(container,'section-child',title,false)];
    const rows=[];for(let i=0;i<stats.length;i+=2)rows.push(rowBlock(stats.slice(i,i+2),title+' metrics'));
    return rows;
  }
  ...
  function logicalBlocks(report){
    ...
    for(const section of report.querySelectorAll(':scope > .rb-section')){
      ...
      for(const child of [...section.children]){
        ...
        if(child.matches('.rb-two')){
          const kids=[...child.children].filter(Boolean);if(kids.length){for(let i=0;i<kids.length;i+=2)push(rowBlock(kids.slice(i,i+2),title+' panels'));continue;}
        }
        ...
        if(tables.length){tables.forEach(t=>push(nodeBlock(t,'section-child',title+' table')));continue;}
        const innerStats=child.matches?.('.rb-stats')?child:null;
        if(innerStats){statRows(innerStats,title).forEach(push);continue;}
  ...
  function wrapItem(item){
    const w=document.createElement('div');w.className='pt-pdf-block-v22';Object.assign(w.style,{width:CAPTURE_WIDTH+'px',height:'auto',overflow:'visible',background:'#fff',boxSizing:'border-box',padding:'0',margin:'0'});
    if(item.type==='row'){
  ...
  async function snapItem(stage,item,index,total){stage.replaceChildren();const wrap=wrapItem(item);stage.appendChild(wrap);await new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)));const r=wrap.getBoundingClientRect();if(r.width<2||r.height<2)return null;status(`Rendering PDF block ${index+1} of ${total}…`);return withTimeout(window.html2canvas(wrap,{scale:1.35,useCORS:true,allowTaint:false,backgroundColor:'#ffffff',logging:false,scrollX:0,scrollY:0,width:Math.ceil(r.width),height:Math.ceil(r.height),windowWidth:CAPTURE_WIDTH}),18000,`PDF capture timed out on ${item.label}.`);}
  ...
  async function build(doc,report,stage){const blocks=logicalBlocks(report),usableW=PAGE_W-MARGIN_X*2,usableBottom=PAGE_H-FOOTER-BOTTOM,fullH=usableBottom-TOP;let y=TOP,page=1;for(let i=0;i<blocks.length;i++){const item=blocks[i],canvas=await snapItem(stage,item,i,blocks.length);if(!canvas)continue;const h=canvas.height*(usableW/canvas.width);let nextH=0;if(item.keepNext&&blocks[i+1]){const nextCanvas=await snapItem(stage,blocks[i+1],i+1,blocks.length);if(nextCanvas)nextH=Math.min(140,nextCanvas.height*(usableW/nextCanvas.width));}
      if(h<=fullH){const need=(y>TOP?GAP:0)+h+(item.keepNext?nextH:0);if(y>TOP&&y+need>usableBottom){doc.addPage();page++;y=TOP;}if(y>TOP)y+=GAP;doc.addImage(canvas.toDataURL('image/jpeg',0.97),'JPEG',MARGIN_X,y,usableW,h,undefined,'FAST');y+=h;}else{if(y>TOP){doc.addPage();page++;y=TOP;}const r=addTall(doc,canvas,usableW,y);page+=r.pagesAdded;y=r.y;}}
    return page;}
  async function generate(){if(running)return false;running=true;const btn=document.getElementById('rbDownloadPdf');let stage=null;if(btn){btn.disabled=true;btn.textContent='Generating PDF…';}try{const source=await withTimeout(currentPreview(),3000,'Current report preview could not be prepared.');await ensureHtml2Canvas();const jsPDF=window.jspdf?.jsPDF;if(!jsPDF)throw new Error('PDF library unavailable.');stage=makeStage();const doc=new jsPDF({unit:'pt',format:'letter',orientation:'portrait',compress:true}),p=profile();doc.setProperties({title:`${PRODUCT} | ${REPORT_TYPE}`,author:[p.full_name,p.company_name].filter(Boolean).join(' - ')||PRODUCT,subject:state?.address||state?.name||REPORT_TYPE,creator:PRODUCT});await build(doc,source,stage);const total=doc.getNumberOfPages();for(let i=1;i<=total;i++){doc.setPage(i);addFooter(doc,i,total,p);}doc.save(filename());status('PDF generated from current preview.');return true;}catch(e){console.error('PropertyThesis PDF export failed',e);status(e?.message||'Unable to generate PDF.');try{alert(e?.message||'Unable to generate PDF.');}catch(_e){}return false;}finally{stage?.remove();running=false;if(btn){btn.disabled=false;btn.textContent='Download PDF';}}}
