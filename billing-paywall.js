'use strict';
(()=>{
  if((window.__propertyThesisBillingV||0)>=8)return;window.__propertyThesisBillingV=8;
  ...
  async function checkout(plan){
    ...
    try{
      ...
      const {data,error}=await cloudClient.functions.invoke('create-checkout',{body:{plan,propertyId}});if(error)throw error;if(!data?.url)throw new Error(data?.error||'Checkout could not be created.');try{window.UnsavedChangeProtection?.markClean?.()}catch(_e){}location.href=data.url
    }catch(e){
  ...
  async function upgrade(plan){
    ...
    }catch(e){localStorage.removeItem('ptBillingUpgradeV1');busy=false;status('Checkout could not open: '+String(e?.message||e));banner('Checkout could not open. Please try again.');}
  }
