...
test('guest route changes select only the current destination, not the green CTA',()=>{
 ...
 const nav={...node(''),dataset:{},querySelector:()=>links[3],querySelectorAll:()=>links};let listings=false,guide=false;
...
test('sample and pricing pages load shared navigation before body rendering',()=>{
 for(const file of ['sample-report-viewer.html','sample-property-card.html','pricing.html']){
  const html=readFileSync(new URL('../'+file,import.meta.url),'utf8');
  assert.ok(html.indexOf('public-navigation.css?v=2')<html.indexOf('</head>'));
  assert.match(html,/<nav class="[^"]*pt-public-navigation/);
  assert.match(html,/<a class="[^"]*pt-free-analysis-cta/);
 }
 assert.match(source,/\.sample-toolbar/);
});
